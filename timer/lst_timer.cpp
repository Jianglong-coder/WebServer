...
void sort_timer_lst::adjust_timer(util_timer *timer)
{
    ...
    if (!tmp || (timer->expire < tmp->expire)) // 超时时间更改后 还是没有后面的时间大  则不用变动
    {
        return;
    }
    // 将定时器从链中摘出 再重新加入
...
void sort_timer_lst::tick()
{
    ...
    while (tmp) // 遍历定时器链表
    {
        if (cur < tmp->expire) // 没到定时器的超时时间 无事发生
        {
            break;
        }
        tmp->cb_func(tmp->user_data); // 超过了该连接中定时器的超时时间 说明该连接为不活跃连接 关闭该连接
        head = tmp->next;             // 从定时器链表中删除该定时器
        if (head)
        {
            head->prev = NULL;
        }
        delete tmp; // 释放该定时器 堆区资源
        tmp = head; // 继续看下一个定时器
    }
}
...
void sort_timer_lst::add_timer(util_timer *timer, util_timer *lst_head)
{
    ...
    while (tmp)
    {
        if (timer->expire < tmp->expire) // 插入链表中间
        {
            prev->next = timer;
            timer->next = tmp;
            tmp->prev = timer;
